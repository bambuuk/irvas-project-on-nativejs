...
window.addEventListener('DOMContentLoaded', function () {
  var modalState = {};
  var ded = new Date();
  ded.setDate(ded.getDate() + 2); // здесь создал переменную с ткущей датой +1 день
...
/** Функция, которая сохраняет данные в state из модального окна расчёта стоимости
 */
...
var images = function images() {
  ...
  workSection = document.querySelector('.works'),
      // переменная, которая отвечает за все картинки
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && target.matches('div.popup')) {
      imgPopup.style.display = 'none';
      /** Если пользователь кликнул на подложку модалки,
...
var timer = function timer(id, deadline) {
  ...
  var setClock = function setClock(selector, endtime) {
    var timer = document.querySelector(selector),
        ...
        timeInterval = setInterval(updateClock, 1000); //Функция, которая определяет, сколько времени осталось до дедлайна
    ...
    function updateClock() {
      ...
      seconds.textContent = addZero(t.seconds); // если визде по 0, то останавливаем таймер
